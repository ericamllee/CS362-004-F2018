...
int councilRoom(struct gameState *state, int currentPlayer, int handPos) {
  //+4 Cards
  for (i = 0; i < 4; i++)
  {
    drawCard(currentPlayer, state);
...
int minion(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2) {
  //+1 action
  state->numActions++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (supplyCount(estate, state) > 0){
    ...
    case minion:
      return minion(state, currentPlayer, handPos, choice1, choice2);
    case steward:
      if (choice1 == 1)
  {
    //+2 cards
